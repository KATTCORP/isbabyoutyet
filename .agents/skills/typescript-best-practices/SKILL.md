...
- **No optional `?` in `projects/baby-outlet/web`.** `workspace/no-optional` requires a present key with `T | null` / `T | undefined`. Do not reintroduce `Partial<{ ... }>` bags to dodge that rule when the caller already has the fields.
